"""Testes do loader do spec bespoke DE-003 (`de003_loader.py`, Milestone 3)."""
...
class TestCarregarEspecificacoesDe003:
    def test_carrega_o_codigo_confirmado(self) -> None:
        specs = carregar_especificacoes_de003()

        codigos = {item["regra"].codigo for item in specs}
        assert codigos == {"DE-003"}

    def test_toda_regra_e_uma_regrade003spec_valida(self) -> None:
        specs = carregar_especificacoes_de003()

        for item in specs:
            assert isinstance(item["regra"], RegraDe003Spec)

    def test_toda_descoberta_e_do_tipo_de003(self) -> None:
        specs = carregar_especificacoes_de003()

        for item in specs:
            assert item["descoberta"]["tipo"] == "de003"
    ...
    def test_nao_repete_nenhum_codigo_ja_migrado(self) -> None:
        ...
        codigos_de003 = {item["regra"].codigo for item in carregar_especificacoes_de003()}

        assert codigos_outros.isdisjoint(codigos_de003)
