...
| 2 | **Saúde dos dados em produção** (pipeline error sem alerta; 20 contas com role inválido `user` passaram 8 dias invisíveis) | `dados-sentinela` | No monitor da VPS: lê `update_log.jsonl` (status≠ok = HIGH), idade máxima por fonte vs cadência esperada do crontab; INTEGRIDADE DE ENUMS no banco (role/status fora do vocabulário válido = achado — caso das 20 personas, 2026-07-30) (prices_20y ≤1 pregão, price_bars cripto ≤5min, events ≤1h...), contagem de linhas anômala (queda >20% dia a dia). Tabela fonte→cadência declarada em spec | S |
