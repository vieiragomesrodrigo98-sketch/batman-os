"""Loader do spec bespoke SRE-006 (`specs/sre006/*.json`) — mesmo padrão
...
_DIR_SPECS = Path(__file__).parent / "specs" / "sre006"


class SpecSre006(TypedDict):
    regra: RegraSre006Spec
    descoberta: dict[str, Any]


def _carregar_arquivo(caminho: Path) -> SpecSre006:
    ...
    return SpecSre006(
        regra=RegraSre006Spec.model_validate(bruto["regra"]), descoberta=bruto["descoberta"]
    )


def carregar_especificacoes_sre006() -> list[SpecSre006]:
