"""Loader do spec bespoke REV-005 (`specs/rev005/*.json`) — mesmo padrão
...
_DIR_SPECS = Path(__file__).parent / "specs" / "rev005"


class SpecRev005(TypedDict):
    regra: RegraRev005Spec
    descoberta: dict[str, Any]


def _carregar_arquivo(caminho: Path) -> SpecRev005:
    ...
    return SpecRev005(
        regra=RegraRev005Spec.model_validate(bruto["regra"]), descoberta=bruto["descoberta"]
    )


def carregar_especificacoes_rev005() -> list[SpecRev005]:
