"""Loader do spec bespoke BE-010 (`specs/be010/*.json`) — mesmo padrão de
`fe001_loader.py`: só 1 código, não generalizado em Skill."""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any, TypedDict

from batman_os.capabilities.rules.be010_dependencia_nao_declarada import RegraBe010Spec
from batman_os.capabilities.rules.lote_01 import SpecInvalido

_DIR_SPECS = Path(__file__).parent / "specs" / "be010"


class SpecBe010(TypedDict):
    regra: RegraBe010Spec
    descoberta: dict[str, Any]


def _carregar_arquivo(caminho: Path) -> SpecBe010:
    bruto = json.loads(caminho.read_text(encoding="utf-8"))
    if "regra" not in bruto or "descoberta" not in bruto:
        raise SpecInvalido(f"{caminho.name}: esperava as chaves 'regra' e 'descoberta'")
    return SpecBe010(
        regra=RegraBe010Spec.model_validate(bruto["regra"]), descoberta=bruto["descoberta"]
    )


def carregar_especificacoes_be010() -> list[SpecBe010]:
    return [_carregar_arquivo(caminho) for caminho in sorted(_DIR_SPECS.glob("*.json"))]
